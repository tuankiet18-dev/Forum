...
// Màu badge dùng opacity để đẹp trên cả nền sáng/tối
...
export const ProblemCard: React.FC<{ problem: Problem }> = ({ problem }) => {
  const difficulty = (problem.difficulty?.toLowerCase() || "medium") as keyof typeof difficultyColors;
  ...
  return (
    <Card className="p-6 flex flex-col h-full cursor-pointer hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-start gap-4 mb-3">
        ...
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${difficultyColors[difficulty]} capitalize whitespace-nowrap`}>
          {difficulty}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="flex items-center gap-4 text-xs text-muted-foreground font-medium">
          <div className="flex items-center gap-1.5">
            <UserOutlined />
            <span>{problem.author?.username || "Unknown"}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <CheckCircleOutlined />
            <span>{problem.solutions || 0} solutions</span>
          </div>
        </div>
