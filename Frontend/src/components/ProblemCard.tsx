...
export const ProblemCard: React.FC<{ problem: Problem }> = ({ problem }) => {
  ...
  return (
    <Link to={`/problems/${problem.id}`} className="block h-full">
        ...
        <div className="flex justify-between items-start gap-4 mb-3">
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${badgeClass} capitalize whitespace-nowrap`}>
            {difficulty}
            </span>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto">
            ...
            <div className="flex items-center gap-1.5">
                <UserOutlined />
                {/* Sửa: Dùng problem.username thay vì problem.author.username */}
                <span>{problem.username}</span> 
            </div>
            <div className="flex items-center gap-1.5">
                <CheckCircleOutlined />
                {/* Sửa: Dùng problem.solutionCount */}
